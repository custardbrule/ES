...
{
    public record Diary(string Name, string Description, string AuthorId, string AuthorName, EDiaryVisibility Visibility) : BaseAuditEntity<Guid>(Guid.NewGuid(), DateTimeOffset.UtcNow)
    {
        ...
        private Diary Apply(InitDiary initDiary) => this with { Id = initDiary.Id, CreatedDate = initDiary.CreatedDate, Name = initDiary.Name, Description = initDiary.Description, AuthorId = initDiary.AuthorId, AuthorName = initDiary.AuthorName, Visibility = initDiary.Visibility };
